...
```bash
pnpm install       # Install dependencies (requires Node >=20.10.0, pnpm >=9)
pnpm run build     # Minify style.css → style.min.css (only if style.css is edited directly)
pnpm run watch     # Watch mode: auto-minify on CSS changes
```
